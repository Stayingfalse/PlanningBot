...
4. To also enable the bot:
   ...
   - **Installation** tab (or **OAuth2 → URL Generator** with the *Guild Install* integration type): scopes `bot` and `applications.commands`; bot permissions `Send Messages`, `Embed Links`, `Use Slash Commands`, `Mention Everyone` (needed so the responder-tag pings actually notify people). Open the generated URL to add the bot to your server. The in-app **Add Bot to Server** button uses this same Guild Install URL (`integration_type=0`), so it works even though the app has no OAuth2 code-grant redirect configured — if you build your own invite URL instead and leave off `integration_type=0`, Discord will reject it with "Integration requires code grant". (Don't use `integration_type=1` — that's *User* Install, and Discord rejects the `bot` scope there with "Invalid scopes provided for user installation".)
